...
- **Version:** 0.18.0 (`pyproject.toml`) · **Framework:** FastMCP (mcp SDK >=1.11.0,<1.16.0), httpx, pydantic, Pillow
...
## Registered tool groups — 98 tools across 16 modules (`aeko_mcp/tools/`)
...
- `docs/aeko-mcp-overview.md` — architecture + token flow; older narrative sections may lag the live 92-tool surface, so trust `aeko_mcp/tools/*.py` for tool details
